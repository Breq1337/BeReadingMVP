      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight" style={{ fontFamily: "var(--font-heading)" }}>
            {t("app.literaryOlympics")}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">{t("app.organizeCompetitions")}</p>
        </div>
        <button
      ...
      {showCreate && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <Trophy className="w-5 h-5 text-warm" /> {t("app.createNewOlympiad")}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("app.olympiadName")}</label>
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("app.baseBook")}</label>
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("app.startDate")}</label>
              <input type="date" className="w-full px-3 py-2.5 rounded-xl border border-border/60 bg-secondary/30 text-sm" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("app.endDate")}</label>
              <input type="date" className="w-full px-3 py-2.5 rounded-xl border border-border/60 bg-secondary/30 text-sm" />
            </div>
          </div>
          <div className="mb-4">
            <label className="text-xs text-muted-foreground mb-2 block">{t("app.participatingClasses")}</label>
          ...
          <div className="mb-4">
            <label className="text-xs text-muted-foreground mb-2 block">{t("app.rounds")}</label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[t("app.lightningQuiz"), t("app.creativeWriting"), t("app.teamDebate"), t("app.artProject")].map((r) => (
      ...
      {olympiads.map((olympiad, oi) => (
        <motion.div key={olympiad.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="bg-gradient-to-r from-warm/5 to-gold/5 p-6 border-b border-border/40">
            <div className="flex items-center justify-between">
              ...
              <Badge className={`${
                ...
                {olympiad.status === "active" ? t("app.inProgressStatus") : olympiad.status === "upcoming" ? t("app.upcoming") : t("app.finished")}
              </Badge>
            </div>
          ...
          <div className="p-6 border-b border-border/40">
            <h3 className="text-sm font-semibold mb-3">{t("app.rounds")}</h3>
          ...
          <div className="p-6">
            <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
              <Medal className="w-4 h-4 text-warm" /> {t("app.ranking")}
            </h3>
